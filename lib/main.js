...
function Logistic() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( mu ) || isnan( mu ) ) {
			throw new TypeError( format( '14d8p,HM', mu ) );
		}
		if ( !isPositive( s ) ) {
			throw new TypeError( format( '14d7c,HN', s ) );
		}
	} else {
		mu = 0.0;
		s = 1.0;
	...
	defineProperty( this, 'mu', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '14d8d,H8', value ) );
			}
			mu = value;
		}
	});
	defineProperty( this, 's', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '14d8k,HE', value ) );
			}
			s = value;
		}
	});
